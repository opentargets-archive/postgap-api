...
const getLocusFiltersSql = ({ g2VMustHaves, g2VScore, r2, gwasPValue }) => {
  ...
  if (g2VScore && g2VScore.length === 2) {
    filtersSql += ` AND (score >= ${g2VScore[0]}) AND (score <= ${
      g2VScore[1]
    })`;
  }
  if (r2 && r2.length === 2) {
  ...
  if (gwasPValue && gwasPValue.length === 2) {
    ...
    filtersSql += ` AND (gwas_pvalue >= ${10 **
      -gwasPValue[1]}) AND (gwas_pvalue <= ${10 ** -gwasPValue[0]})`;
  }
  return filtersSql;
};
...
const resolveGeneVariants = ({ common }, args, { db, geneLocationsCache }) => {
  ...
  const geneVariantsSql = `
    ...
    ${orderBySql}
    `;
  const geneVariantsQuery = db
    .all(geneVariantsSql, params)
    .then(geneVariants => {
      const start = Date.now();
...
const resolveMaxGwasPValue = ({ common }, args, { db }) => {
  ...
  const maxGwasPValueSql = `
    ...
    ${unfilteredWhere}
    `;
  const maxGwasPValueQuery = db.all(maxGwasPValueSql, params).then(rows => {
